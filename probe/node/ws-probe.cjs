...
function encBig(n) {
  ...
  const b = []
  do {
    b.push(Number(v & 0x7fn) | (v > 0x7fn ? 0x80 : 0))
    v >>= 7n
  } while (v > 0n)
...
function buildAck(logId, internalExt) {
  return Buffer.concat([field(2, 0, encBig(logId)), strField(7, 'ack'), strField(8, internalExt)])
}
...
async function main() {
  ...
  ws.on('upgrade', res => {
    console.log(
      `[4] 握手 HTTP ${res.statusCode}${res.statusCode === 101 ? ' ✅ 签名被接受' : ' ❌ ' + JSON.stringify(res.headers)}`,
    )
  })
  ws.on('open', () => {
    console.log(
      `[4] WS 已打开，开始收帧（最多 ${DURATION_SEC} 秒 / 收到弹幕即退出；NO_ACK=${NO_ACK} VERBOSE=${VERBOSE}）`,
    )
    ...
    setTimeout(() => {
      console.log(`[!] ${DURATION_SEC} 秒到，主动结束`)
      ws.close()
    }, DURATION_SEC * 1000)
  })
  ...
  ws.on('message', data => {
    ...
    let pf
    try {
      pf = parsePushFrame(buf)
    } catch (e) {
      console.log('[!] PushFrame 解析失败：' + e.message)
      return
    }
    if (!pf.payload) {
      console.log(`[帧] 无 f8 payload，字段=${pf.fieldNums}`)
      return
    }
    ...
    if (magic >= 0) {
      try {
        raw = zlib.gunzipSync(raw.subarray(magic))
      } catch (e) {
        console.log('[!] gunzip 失败：' + e.message)
        return
      }
    }
    let resp
    try {
      ...
      console.log('[!] Response 解析失败：' + e.message)
      return
    }
    ...
    for (const msg of resp.messages) {
      ...
      if (process.env.PROBE_USER === '1') {
        ...
        const userBytes = msg.method === 'WebcastGiftMessage' ? byNum(uf, 7)?.bytes : byNum(uf, 2)?.bytes
        if (
          userBytes &&
          [
            'WebcastChatMessage',
      ...
      if (
        ...
        ['WebcastChatMessage', 'WebcastMemberMessage', 'WebcastRoomUserSeqMessage', 'WebcastLikeMessage'].includes(
          msg.method,
        )
      ) {
    ...
    if (resp.needAck && !NO_ACK) {
      ...
      try {
        ...
        console.log('[!] ack 发送失败：' + e.message)
      }
    }
    if (gotGift) {
      console.log('[✓] 抓到礼物，2 秒后结束')
      setTimeout(() => ws.close(), 2000)
      return
    }
    const chatStop = Number(process.env.CHAT_STOP || 3)
    if (chatSamples >= chatStop) {
      console.log(`[✓] 已收到 ${chatStop} 条弹幕，主动结束`)
      ws.close()
    }
  })
}
...
function dumpFields(buf, depth = 0, maxDepth = 5) {
  ...
  for (const f of fields) {
    if (f.wire === 0) {
      lines.push(`${pad}f${f.num}/varint = ${f.varint}`)
      continue
    }
    if (!f.bytes) continue
